...
Runtime mode resolution order: `runtime_mode_override` arg → `SKILL_RUNTIME_MODE` env var → skill frontmatter `metadata.runtime` → default `legacy`. All production capture/proposal skills (`workload-analyzer`, `data-analyzer`, `huashu-design`, plus the tools-mode govcon skills) declare `metadata.runtime: tools`; legacy mode remains for meta/dev skills (`tdd`, `grill-me`, `handoff`, etc.) until migrated.
