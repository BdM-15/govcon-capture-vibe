"""LangGraph step pipeline: retrieve (main LLM) -> finalize (platform) -> conditional retry."""
...
from src.skills.platform_step_finalize import finalize_step_handoff
...
class StepPipelineState(TypedDict, total=False):
    chain_id: str
    chain_dir: str
    step_id: str
    retry_count: int
...
def use_step_pipeline(step: ChainStepSpec) -> bool:
    ctx = step.context or {}
    return bool(ctx.get("langgraph_step_pipeline") or ctx.get("langgraph_eval_pipeline"))


def build_step_pipeline_graph():
    graph = StateGraph(StepPipelineState)
...
def _route_after_finalize(state: StepPipelineState) -> str:
...
async def _retrieve_node(state: StepPipelineState, *, config: RunnableConfig) -> dict[str, Any]:
    ...
    emit_chain_event(
        ...
        ChainEvent(
            ...
            phase="step_pipeline",
            event="step_retrieve_started",
            summary=f"{step.id} retrieve pass {retry + 1}",
            step_id=step.id,
            skill=step.skill,
            status="running",
            detail={"retry": retry},
    ...
    if result is None:
        return {
            "status": "failed",
            "gate_issues": [f"{step.id} retrieve returned no result"],
            "warnings": warnings,
            "retry_count": retry,
        }
...
async def _finalize_node(state: StepPipelineState, *, config: RunnableConfig) -> dict[str, Any]:
    cfg = config.get("configurable") or {}
    chain_dir = Path(state["chain_dir"])
    chain = ChainRunState.model_validate(cfg["chain"])
    step: ChainStepSpec = cfg["step"]
    ...
    if not run_dir:
        return {
            "status": "failed",
            "gate_issues": [f"{step.id} finalize: missing run_dir from retrieve"],
        }
    ...
    emit_chain_event(
        ...
        ChainEvent(
            chain_id=chain.chain_id,
            phase="step_pipeline",
            event="step_finalize_started",
            summary=f"{step.id} platform finalize + gate",
            step_id=step.id,
            skill=step.skill,
            status="running",
        ),
    ...
    workspace_root = Path(cfg["workspace_root"])
    finalize = await finalize_step_handoff(
        skill=step.skill,
    ...
    if finalize.get("passed"):
        emit_chain_event(
            ...
            ChainEvent(
                chain_id=chain.chain_id,
                phase="step_pipeline",
                event="step_gate_passed",
                summary=f"{step.id} passed platform gate",
                step_id=step.id,
                skill=step.skill,
                status="completed",
            ),
    ...
    if retriable and retry < _MAX_RETRIEVE_RETRIES and not compiler_step:
        emit_chain_event(
            ...
            ChainEvent(
                chain_id=chain.chain_id,
                phase="step_pipeline",
                event="step_gate_retry",
                summary=f"{step.id} gap — retrieve retry {retry + 1}",
                step_id=step.id,
                skill=step.skill,
                status="running",
...
async def run_step_pipeline_step(
    *,
    chain: ChainRunState,
    step: ChainStepSpec,
    chain_dir: Path,
    ...
    """Run one chain step as LangGraph subgraph."""
    graph = build_step_pipeline_graph()
    ...
    if status == "completed" and run_id and run_dir:
        result = SkillInvocationResult(
            skill=step.skill,
            workspace=chain.workspace,
            response=str(final_state.get("response") or f"{step.id} pipeline complete"),
            entities_used=[],
            warnings=warnings,
        ...
        return StepPipelineOutcome(step_id=step.id, result=result)

    error = "; ".join(gate_issues[:6]) if gate_issues else f"{step.id} pipeline {status}"
    if len(gate_issues) > 6:
        error += f"; …and {len(gate_issues) - 6} more"
    return StepPipelineOutcome(step_id=step.id, result=None, error=error)