...
def compose_system_prompt(
    ...
    return (
        ...
        "deliverables with `write_file`. For public-web context use "
        "`web_search`, `web_fetch`, or `web_research` — label those claims "
        "with external provenance, separate from solicitation `[chunk-…]` citations.\n"
        ...
        "- Follow the skill's depth, retrieval, and stopping rules in Skill "
        "Instructions — they override generic efficiency heuristics. Do not "
        "stop early when the skill defines minimum tool use or output depth.\n"
        "- When those requirements are satisfied, stop calling tools and "
        "produce your final answer. The final assistant message (no tool "
        "calls) is what the user sees.\n"
