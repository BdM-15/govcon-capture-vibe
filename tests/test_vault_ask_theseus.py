...
def test_vault_ask_helpers_exported():
    """theseusVaultAskTheseus and theseusVaultSaveAsNote in vault helpers."""
    text = _VAULT_HELPERS.read_text(encoding="utf-8")
    assert "theseusVaultAskTheseus" in text
    assert "theseusVaultSaveAsNote" in text
...
def test_vault_ask_delegates_wired():
    """vaultAskTheseus and vaultSaveAsNote delegates in app delegates."""
    text = _DELEGATES.read_text(encoding="utf-8")
    assert "vaultAskTheseus" in text
    assert "vaultSaveAsNote" in text
...
def test_ask_theseus_button_in_right_pane():
    """Ask Theseus button present in HTML and calls vaultAskTheseus."""
    text = _INDEX.read_text(encoding="utf-8")
    assert "vaultAskTheseus" in text


def test_ask_answer_display_in_right_pane():
    """Answer display area using vaultAskAnswer present in HTML."""
    text = _INDEX.read_text(encoding="utf-8")
    assert "vaultAskAnswer" in text


def test_save_as_note_button_in_html():
    """Save as Note button present in HTML and calls vaultSaveAsNote."""
    text = _INDEX.read_text(encoding="utf-8")
    assert "vaultSaveAsNote" in text
