...
class TestEntityProposalUiState:
    def test_entity_proposal_state_vars_present(self):
        src_text = Path("src/ui/static/app/theseus-state-helpers.js").read_text(encoding="utf-8")
        assert "vaultEntityProposals" in src_text
        assert "vaultEntityLoading" in src_text

    def test_entity_extract_helper_exported(self):
        src_text = Path("src/ui/static/app/theseus-vault-helpers.js").read_text(encoding="utf-8")
        assert "theseusVaultExtractEntities" in src_text

    def test_entity_accept_helper_exported(self):
        src_text = Path("src/ui/static/app/theseus-vault-helpers.js").read_text(encoding="utf-8")
        assert "theseusVaultAcceptEntities" in src_text

    def test_entity_delegates_wired(self):
        src_text = Path("src/ui/static/app/theseus-app-delegates.js").read_text(encoding="utf-8")
        assert "vaultExtractEntities" in src_text
        assert "vaultAcceptEntities" in src_text

    def test_entity_proposals_pane_in_html(self):
        html = Path("src/ui/static/index.html").read_text(encoding="utf-8")
        assert "vaultEntityProposals" in html

    def test_extract_entities_button_in_html(self):
        html = Path("src/ui/static/index.html").read_text(encoding="utf-8")
        assert "vaultExtractEntities" in html
...
class TestAcceptSelectedDisabledWithoutWorkspace:
    def test_accept_button_disabled_without_workspace(self):
        html = Path("src/ui/static/index.html").read_text(encoding="utf-8")
        assert "vaultAcceptEntities" in html
        # Must have some disabled / condition tied to workspace
        assert "stats.workspace" in html or "activeWorkspace" in html or "workspace" in html
